/*! Readme: /Terms.aspx . Rel: 10/9/2014 7:46:44 AM UTC *//*
...
* @version 0.9.5
*/
(function(c){var d=c(window);c.fn.asynchImageLoader=c.fn.jail=function(b){b=c.extend({timeout:10,effect:false,speed:400,selector:null,offset:0,event:"load+scroll",callback:jQuery.noop,callbackAfterEachImage:jQuery.noop,placeholder:false,container:window},b);var a=this;c.jail.initialStack=this;this.data("triggerEl",(b.selector)?c(b.selector):d);if(b.placeholder!==false){a.each(function(){c(this).attr("src",b.placeholder)})}if(/^load/.test(b.event)){c.asynchImageLoader.later.call(this,b)}else{c.asynchImageLoader.onEvent.call(this,b,a)}return this};c.asynchImageLoader=c.jail={_purgeStack:function(a){var b=0;while(true){if(b===a.length){break}else{if(a[b].getAttribute("data-href")){b++}else{a.splice(b,1)}}}},_loadOnEvent:function(h){var e=c(this),b=h.data.options,a=h.data.images;c.asynchImageLoader._loadImage(b,e);e.unbind(b.event,c.asynchImageLoader._loadOnEvent);c.asynchImageLoader._purgeStack(a);if(!!b.callback){c.asynchImageLoader._purgeStack(c.jail.initialStack);c.asynchImageLoader._launchCallback(c.jail.initialStack,b)}},_bufferedEventListener:function(h){var a=h.data.images,b=h.data.options,e=a.data("triggerEl");clearTimeout(a.data("poller"));a.data("poller",setTimeout(function(){a.each(function f(){c.asynchImageLoader._loadImageIfVisible(b,this,e)});c.asynchImageLoader._purgeStack(a);if(!!b.callback){c.asynchImageLoader._purgeStack(c.jail.initialStack);c.asynchImageLoader._launchCallback(c.jail.initialStack,b)}},b.timeout))},onEvent:function(b,a){a=a||this;if(b.event==="scroll"||b.selector){var f=a.data("triggerEl");if(a.length>0){f.bind(b.event,{images:a,options:b},c.asynchImageLoader._bufferedEventListener);if(b.event==="scroll"||!b.selector){d.resize({images:a,options:b},c.asynchImageLoader._bufferedEventListener)}return}else{if(!!f){f.unbind(b.event,c.asynchImageLoader._bufferedEventListener)}}}else{a.bind(b.event,{options:b,images:a},c.asynchImageLoader._loadOnEvent)}},later:function(b){var a=this;if(b.event==="load"){a.each(function(){c.asynchImageLoader._loadImageIfVisible(b,this,a.data("triggerEl"))})}c.asynchImageLoader._purgeStack(a);c.asynchImageLoader._launchCallback(a,b);setTimeout(function(){if(b.event==="load"){a.each(function(){c.asynchImageLoader._loadImage(b,c(this))})}else{a.each(function(){c.asynchImageLoader._loadImageIfVisible(b,this,a.data("triggerEl"))})}c.asynchImageLoader._purgeStack(a);c.asynchImageLoader._launchCallback(a,b);if(b.event==="load+scroll"){b.event="scroll";c.asynchImageLoader.onEvent(b,a)}},b.timeout)},_launchCallback:function(a,b){if(a.length===0&&!c.jail.isCallback){b.callback.call(this,b);c.jail.isCallback=true}},_loadImageIfVisible:function(b,j,i){var h=c(j),a=(/scroll/i.test(b.event))?i:d;if(c.asynchImageLoader._isInTheScreen(a,h,b.offset)){c.asynchImageLoader._loadImage(b,h)}},_isInTheScreen:function(t,a,r){var p=t[0]===window,x=(p?{top:0,left:0}:t.offset()),q=x.top+(p?t.scrollTop():0),s=x.left+(p?t.scrollLeft():0),o=s+t.width(),u=q+t.height(),w=a.offset(),v=a.width(),b=a.height();return(q-r)<=(w.top+b)&&(u+r)>=w.top&&(s-r)<=(w.left+v)&&(o+r)>=w.left},_loadImage:function(a,b){b.hide();b.attr("src",b.attr("data-href"));b.removeAttr("data-href");if(a.effect){if(a.speed){b[a.effect](a.speed)}else{b[a.effect]()}}else{b.show()}a.callbackAfterEachImage.call(this,a)}}}(jQuery));/*
 ...
 *
 **/
jQuery.fn.extend({everyTime:function(b,c,a,d){return this.each(function(){jQuery.timer.add(this,b,c,a,d)})},oneTime:function(b,c,a){return this.each(function(){jQuery.timer.add(this,b,c,a,1)})},stopTime:function(b,a){return this.each(function(){jQuery.timer.remove(this,b,a)})}});jQuery.extend({timer:{global:[],guid:1,dataKey:"jQuery.timer",regex:/^([0-9]+(?:\.[0-9]*)?)\s*(.*s)?$/,powers:{ms:1,cs:10,ds:100,s:1000,das:10000,hs:100000,ks:1000000},timeParse:function(d){if(d==undefined||d==null){return null}var c=this.regex.exec(jQuery.trim(d.toString()));if(c[2]){var b=parseFloat(c[1]);var a=this.powers[c[2]]||1;return b*a}else{return d}},add:function(b,e,f,c,h){var a=0;if(jQuery.isFunction(f)){if(!h){h=c}c=f;f=e}e=jQuery.timer.timeParse(e);if(typeof e!="number"||isNaN(e)||e<0){return}if(typeof h!="number"||isNaN(h)||h<0){h=0}h=h||0;var g=jQuery.data(b,this.dataKey)||jQuery.data(b,this.dataKey,{});if(!g[f]){g[f]={}}c.timerID=c.timerID||this.guid++;var d=function(){if((++a>h&&h!==0)||c.call(b,a)===false){jQuery.timer.remove(b,f,c)}};d.timerID=c.timerID;if(!g[f][c.timerID]){g[f][c.timerID]=window.setInterval(d,e)}this.global.push(b)},remove:function(a,c,b){var e=jQuery.data(a,this.dataKey),d;if(e){if(!c){for(c in e){this.remove(a,c,b)}}else{if(e[c]){if(b){if(b.timerID){window.clearInterval(e[c][b.timerID]);delete e[c][b.timerID]}}else{for(var b in e[c]){window.clearInterval(e[c][b]);delete e[c][b]}}for(d in e[c]){break}if(!d){d=null;delete e[c]}}}for(d in e){break}if(!d){jQuery.removeData(a,this.dataKey)}}}}});jQuery(window).bind("unload",function(){jQuery.each(jQuery.timer.global,function(a,b){jQuery.timer.remove(b)})});/* Vera VAF 1.0 license: https://github.com/sparkledb/vera */
$(function(){$.fn.extend({fromISO8601:function(c){return a(c)},toIso8601Date:function(c){return b(c)}});function a(f){var d=f.split("Z");var g=d[0].split("T"),e=g[0].split("-"),i=g[1].split("."),k=i[0].split(":"),j=k[2].split("."),h=Number(k[0]),c=new Date;c.setUTCFullYear(Number(e[0]));c.setUTCMonth(Number(e[1])-1);c.setUTCDate(Number(e[2]));c.setUTCHours(Number(h));c.setUTCMinutes(Number(k[1]));c.setUTCSeconds(Number(j[0]));if(j[1]){c.setUTCMilliseconds(Number(j[1]))}return c}function b(e){var i=e.getUTCFullYear();if(i<2000){i=i+1900}var g=e.getUTCMonth()+1;var c=e.getUTCDate();var d=e.getUTCHours();var f=e.getUTCMinutes();var h=e.getUTCSeconds();if(g<=9){g="0"+g}if(c<=9){c="0"+c}if(d<=9){d="0"+d}if(f<=9){f="0"+f}if(h<=9){h="0"+h}time=i+"-"+g+"-"+c+"T"+d+":"+f+":"+h;return time}});/* Vera VAF 1.0 license: https://github.com/sparkledb/vera */
$(function(){$.fn.extend({timeAgo:function(){i(this)}});var h=["zero","one","two","three","four","five","six","seven","eight","nine","ten","eleven","twelve"];var b=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];var j=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];var k=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function a(l,m){l.oneTime(m,function(){i($(this))})}function c(l){return b[l.getDay()]}function d(l){return j[l.getDay()]}function e(l){return k[l.getMonth()]}function f(l){var n=l.getMinutes();if(n.toString().length==1){n="0"+n}var m=l.getHours();if(m.toString().length==1){m="0"+m}return m+":"+n}function g(l){if((l+1)>h.length){return l}return h[l]}function i(l){if(l.length==0){return}var n=l.attr("datetime");if(n==undefined||n==null){return}var m=$.fn.fromISO8601(n);l.attr("title",m.toGMTString());var u=new Date();var q=u.getTime()-m.getTime();var s=parseInt(q/60000);var r=parseInt(s/60);var o=parseInt(r/24);var v=parseInt(o/7);if(o>30){l.html(d(m)+", "+m.getDate()+" "+e(m)+" "+m.getFullYear())}else{if(v>=1){if(v==1){var p=o-7;if(p==0){l.html("one week ago at "+f(m))}else{l.html(c(m)+" at "+f(m))}}else{l.html(g(v)+" weeks ago at "+f(m))}a(l,86400000)}else{if(o==1){l.html("yesterday at "+f(m));a(l,3600000)}else{if(o>1){l.html(c(m)+" at "+f(m));a(l,3600000)}else{if(r>=1){if(r==1){var t=s-60;if(t==0){l.html("one hour ago")}else{l.html("one hour "+g(t)+" minutes ago")}}else{l.html(g(r)+" hours ago at "+f(m))}}else{if(s>=1){if(s==1){l.html("one minute ago")}else{l.html(g(s)+" minutes ago")}}else{l.html("just now")}}a(l,60000)}}}}}});/* Vera VAF 1.0 license: https://github.com/sparkledb/vera */
jQuery.fn.preventDoubleSubmission=function(){$(this).on("submit",function(b){var a=$(this);if(a.data("submitted")===true){b.preventDefault()}else{a.data("submitted",true)}});return this};/* Vera VAF 1.0 license: https://github.com/sparkledb/vera */
$(function(){$.fn.extend({callServiceInterface:function(g,f,d,h,e,c){return b(g,f,d,h,e,c)}});$.fn.extend({getFromRemoteClient:function(c,h,g,e,i,f,d,j){return a(c,h,g,e,i,f,d,j)}});function b(g,f,d,h,e,c){var i="/Interfaces/"+g+".svc/"+f;return $.ajax({url:i,data:d,type:"POST",contentType:"application/json",timeout:5000,dataType:"json",processData:false,success:h,error:e}).always(c)}function a(c,h,g,e,i,f,d,j){var k=c+"/Interfaces/"+h+".svc/"+g;return $.ajax({url:k,data:e,type:"GET",contentType:"application/json",timeout:j,dataType:"jsonp",processData:false,success:i,error:f}).always(d)}});/* Vera VAF 1.0 license: https://github.com/sparkledb/vera */
$(document).ready(function(){var h=[];var m=[];var g=true;var k=false;var i=0;var n="";function e(){var s=$("a.rssIconHover");s.css("opacity","0.0");s.css("display","block");s.hover(function(){$(this).stop(true,false);$(this).animate({opacity:"1.0"},400)},function(){$(this).stop(true,false);$(this).animate({opacity:"0.0"},400)})}e();var f=$("input.searchBox");function d(t){var s="";if(t!=null&&$.trim(t)!=""){if(h[t]==null){h[t]=true;s='<figure><img data-href="'+t+'" src="/Images/blank.gif" class="queryResultJailImage" alt="" /></figure>'}else{s='<figure><img alt="" src="'+t+'" /></figure>'}}return s}function p(x,v){var y=$("#searchResult");i=x.Items.length;if(x.Items.length>0){var s;var z;if(x.NumberOfHits==x.Items.length){s=true;if(x.NumberOfHits==1){z="Found 1 page:"}else{z="Found "+x.NumberOfHits+" pages:"}}else{s=false;z="Showing the top "+x.Items.length+" of "+x.NumberOfHits+" pages:"}var w='<article><header><h1 class="searchResultHitsText">'+z+"</h1></header><section>";for(var t=0;t<x.Items.length;t++){var u=d(x.Items[t].ImageUrl);w+='<div class="searchResultArticle"><article><header><h1><a href="'+x.Items[t].Url+'">'+x.Items[t].Title+'</a></h1></header><div class="searchResultText"><a href="'+x.Items[t].Url+'">'+u+"<span>"+x.Items[t].Text+"</span></a></div></article></div>"}w+='</section><div class="searchResultFooter clear"><footer><a id="moreQueryResults" href="#">More...</a><a class="queryHelp" href="/Help/search-query-syntax.aspx">Query syntax guide</a></footer></div></article>';y.html(w);if(!v){$("img.queryResultJailImage").jail({effect:"fadeIn"})}if(s){$("#moreQueryResults").hide()}else{$("#moreQueryResults").click(function(){r(f.val())})}y.fadeIn(200)}else{y.fadeOut(200)}}function l(){k=false;$("#loadingIcon").hide();$("#clearIcon").show();if(n!=""){o(n)}}var j="";function o(s){if(s==j){return}j=s;n="";s=$.trim(s);if(m[s]==null){k=true;$("#loadingIcon").show();$("#clearIcon").hide();$.fn.callServiceInterface("RestApi","Query",'{"queryTerms":"'+s.replace('"','"')+'"}',function(u){var t=$.parseJSON(u);p(t,false);m[s]=t},function(){},function(){l()})}else{p(m[s],true)}}function a(t){if(t.val().length==0){g=true;$("#searchIcon").animate({color:"#aaaaaa"});$("#searchBoxBackground").animate({opacity:0.5});$("#searchBox").animate({width:"12.5em"});var s=$("div.level1MenuContainer > div.level1Menu > div.menu > ul > li:first-child > a");$("div.level1MenuContainer > div.level1Menu > div.menu > ul > li > a").each(function(){if($(this)[0]!=s[0]){$(this).animate({"padding-left":"1em","padding-right":"1em"})}})}var u=$("#searchResult");u.stop(true,true);u.fadeOut(200)}function c(){g=false;$("#searchIcon").animate({color:"#000000"});$("#searchBoxBackground").animate({opacity:1});$("#searchBox").animate({width:"17.857em"});var s=$("div.level1MenuContainer > div.level1Menu > div.menu > ul > li:first-child > a");$("div.level1MenuContainer > div.level1Menu > div.menu > ul > li > a").each(function(){if($(this)[0]!=s[0]){$(this).animate({"padding-left":"0.5em","padding-right":"0.5em"})}})}function b(s){if(s.val().length==0){c()}if(i>0){var t=$("#searchResult");t.stop(true,true);t.fadeIn(200)}}function q(s){if(g){$("#clearIcon").hide()}else{if(s.val().length==0){$("#clearIcon").animate({opacity:0},"fast")}else{$("#clearIcon").animate({opacity:1},"fast")}}}function r(s){window.location.href="/Search.aspx?query="+encodeURIComponent(s)}$("#helpText").fadeIn(1000);f.keypress(function(t){var s=(t.keyCode?t.keyCode:t.which);if(s==10||s==13){r(f.val());t.preventDefault()}});q(f);if(f.val().length>0){c()}f.focus(function(){b($(this))});f.focusout(function(){if(!k){a($(this))}});f.keyup(function(){if(!k){q($(this));if(f.val().length>0){o(f.val())}}else{if(f.val().length>0){n=f.val()}}});$("#searchIcon").click(function(){if(f.val().length>0){r(f.val())}else{f.focus()}});$("#clearIcon").click(function(){f.val("");q($(this));i=0;f.focus()})});/* Vera VAF 1.0 license: https://github.com/sparkledb/vera */
function IsEmail(a){var b=/^\S+@\S+\.\S+$/;return b.test(a)};/* Vera VAF 1.0 license: https://github.com/sparkledb/vera */
$(document).ready(function(){$('input[type="email"]').each(function(i){var h=$(this).attr("id");var k=$(this).parent();var j=$(this);var g=k.find("#email-envelope");var m=k.find("#email-remove");var f=k.find("#email-background");var l=k.find("#email-placeholder");c(j,g,f,m,l)});function a(h,g,f){if(h===undefined){return}if(h.val().length==0){f.animate({opacity:0.5});g.stop(true);g.animate({color:"#cccccc"},"fast")}}function b(h,g,f){if(h===undefined){return}if(h.val().length==0){f.animate({opacity:1});g.stop(true);g.animate({color:"#000000"},"fast")}}function e(f,h){if(f===undefined){return}var g=f.val().length==0?0:1;h.stop(true);h.animate({opacity:g},"fast")}function d(g,f,h){e(g,h);var i;if(g.val().length==0){i="#000000"}else{i=IsEmail(g.val())?"#00cc00":"#f89922"}f.stop(true);f.animate({color:i},"fast")}function c(h,g,f,j,i){if(h===undefined){return}j.click(function(k){k.stopPropagation();h.val("");e(h,$(this));lastNumberOfHits=0;h.focus();d($(this),g,j)});e(h,j);h.attr("placeholder","");if(h.val().length>0){b()}else{i.show()}h.focus(function(){b($(this),g,f)});h.focusout(function(){a($(this),g,f)});h.keyup(function(){d($(this),g,j)})}});/* Vera VAF 1.0 license: https://github.com/sparkledb/vera */
$(document).ready(function(){function a(){var b=750;var g=-6.6;var f=1.3;var c=14;var d=$('button[type="submit"]');var e=Math.floor(d.width()*f*1.25-Math.abs(g)*c);d.animate({backgroundPosition:e+"px"},b,function(){$(this).css("background-position",g+"em")})}setInterval(function(){a()},5750)});/* Vera VAF 1.0 license: https://github.com/sparkledb/vera */
$(document).ready(function(){var d=$(window);var c=$("html, body");var a=$(document);var b=$("form");$("img.jail").show().jail({effect:"fadeIn"});$("time.timeAgo").each(function(){$(this).timeAgo()});$("div.inputBox").each(function(){$(this).inputBox()})});