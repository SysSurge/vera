...
{
    public partial class NewsPage : PageTemplateBase
    {
        ...
        {
            litTitle.Text = new GraphicUtilities().ChangeToJailImages(page.Title);
        }

        private void SetArticleIngress(PageEntity page)
        {
            litIngress.Text = new GraphicUtilities().ChangeToJailImages(page.Ingress);
        }
        ...
        private void SetExternalResources(PageEntity page)
        {
            var markup = new StringBuilder();
            ...
            {
                var urls = page.Css.Split(',');

                foreach (var normalizedUri in urls.Select(subject => Regex.Replace(subject, @"\s", String.Empty)))
                    markup.AppendFormat("<link rel=\"stylesheet\" href=\"{0}\" type=\"text/css\" />",
                        HttpUtility.UrlEncode(normalizedUri));
            }

            // Get page script files
            if (!String.IsNullOrWhiteSpace(page.Scripts))
            {
                var urls = page.Scripts.Split(',');

                foreach (var normalizedUri in urls.Select(subject => Regex.Replace(subject, @"\s", String.Empty)))
                    markup.AppendFormat("<script src=\"{0}\"></script>",
                        HttpUtility.UrlEncode(normalizedUri));
            }

            litExternalResources.Text = markup.ToString();
        }
